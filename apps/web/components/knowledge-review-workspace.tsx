...
export function KnowledgeReviewWorkspace({ proposals }: { proposals: KnowledgeProposal[] }) {
  ...
  const types = [...new Set(proposals.map((item) => item.proposal_type))].sort();
  const visible = type === "all" ? proposals : proposals.filter((item) => item.proposal_type === type);
