...
export function KnowledgeReviewWorkspace({ proposals }: { proposals: KnowledgeProposal[] }) {
  ...
  return <div>
    ...
    <p className="mb-4 text-xs leading-5 text-stone-500">{proposals.length} {proposals.length === 1 ? "knowledge proposal" : "knowledge proposals"} pending. Each action resolves only the proposal shown; it does not mark the source document reviewed.</p>
    <div className="divide-y divide-stone-200 border-y border-stone-200">
      {proposals.map((item) => <article key={item.id} className="min-w-0 py-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex gap-1">
            <button disabled={busy === item.id} onClick={() => decide(item, "create")} className="rounded-lg bg-stone-900 p-2 text-white" aria-label="Accept this knowledge proposal"><Check className="size-4" /></button>
            ...
            <button disabled={busy === item.id} onClick={() => decide(item, "reject")} className="rounded-lg border border-stone-200 p-2 text-stone-400" aria-label="Reject this knowledge proposal"><X className="size-4" /></button>
          </div>
        </div>
