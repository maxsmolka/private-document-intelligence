  ...
  return (
    <>
      <div className="flex flex-col gap-3 border-b border-stone-200 py-4 sm:flex-row">
        <label className="relative flex-1"><span className="sr-only">Filter the current document list by title or filename</span><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-stone-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Filter this list by title or filename…" className="h-9 w-full rounded-lg border border-stone-200 bg-white pl-9 pr-3 text-sm outline-none transition placeholder:text-stone-400 focus:border-stone-400 focus:ring-2 focus:ring-stone-100" /></label>
