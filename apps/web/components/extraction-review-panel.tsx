...
function Version({ item, title }: { item: ExtractionVersion; title: string }) {
  return (
  ...
  return (
    <section className="mt-6 rounded-2xl border border-violet-200 bg-white p-5 shadow-sm">
      <div className="flex items-start gap-3">
        ...
        <div><h2 className="text-sm font-semibold text-stone-900">Extraction review</h2><p className="mt-1 text-xs leading-5 text-stone-500">The current text remains searchable until you explicitly promote the PDI result.</p></div>
      </div>
      <div className="mt-4 grid gap-3 sm:grid-cols-2"><Version item={current} title="Current extraction" /><Version item={candidate} title="PDI candidate" /></div>
      ...
      <div className="mt-4 flex gap-2"><Button variant="secondary" onClick={() => void keepCurrent()} disabled={busy || !comparison}>Keep current</Button><Button onClick={() => void promote()} disabled={busy || !comparison}>{busy ? "Saving…" : "Promote PDI extraction"}</Button></div>
    </section>
  );
}
