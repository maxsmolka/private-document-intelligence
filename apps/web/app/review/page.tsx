...
export default async function ReviewPage({ searchParams }: { searchParams: Promise<{ id?: string }> }) {
  ...
  return <div className="mx-auto max-w-[1500px] px-5 py-8 md:px-8"><div className="mb-7 flex items-end justify-between gap-4"><div className="min-w-0"><div className="flex items-center gap-2 text-sm font-medium text-stone-500"><FileCheck2 className="size-4" />Metadata document review queue</div><h1 className="mt-2 break-words text-2xl font-semibold tracking-[-0.025em] text-stone-950" title={detail.document.title}>{detail.document.title}</h1></div><span className="shrink-0 rounded-full border border-stone-200 bg-white px-3 py-1 text-xs text-stone-500">{queue.total} {queue.total === 1 ? "document" : "documents"} remaining</span></div><ReviewWorkspace key={selected} detail={detail} queue={queue.items} /></div>;
}
