...
`pdi export TARGET` writes explicit JSON arrays for documents, assets, metadata history, intelligence, proposals, organizations, contracts, relations, events, deadlines, actions, knowledge, tags, notes, saved searches, in-app reminder notifications, and migration provenance. Immutable originals are copied under `originals/`; a manifest and SHA-256 list cover the export.
