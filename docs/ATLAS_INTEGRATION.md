...
Atlas integration itself is not implemented in Milestone 6. PDI does not provide chat, RAG answer generation, agents, or cross-domain reasoning. A future Atlas instance should receive only the three read scopes it needs, cite PDI UUID/page provenance, tolerate pagination, and keep derived reasoning outside PDI.
