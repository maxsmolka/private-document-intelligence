...
Images are published as `ghcr.io/maxsmolka/private-document-intelligence/backend` and `/web` with the full release, `1.1`, `1`, and `latest` tags. Deployments should pin the full version or digest. v1.1 supports `linux/amd64`; additional architectures require native OCR and full runtime qualification.
