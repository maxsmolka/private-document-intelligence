...
Images are published as `ghcr.io/maxsmolka/private-document-intelligence/backend` and `/web` with `1.0.0`, `1.0`, `1`, and `latest` tags. Deployments should pin the full version or digest. v1.0 supports `linux/amd64`; additional architectures require native OCR and full runtime qualification.
